...
export default function FlujoPage() {
  ...
  useEffect(() => {
    try {
      ...
      setAsesorName(params.get('asesorName') || 'Vicente Torres');
    ...
    <div style={{ minHeight: '100vh', background: '#f0f7ff', fontFamily: 'system-ui,sans-serif', color: '#0f2957' }}>
      ...
      <main style={{ padding: '20px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', margin: '28px 0 8px' }}>
          <a
            href={`https://wa.me/${asesorPhone}?text=${encodeURIComponent(`Hola ${asesorName}! Quiero avanzar con la reserva. te paso los numeros de teléfono en breve`)}`}
          ...
          <a
            href={`https://wa.me/${asesorPhone}?text=${encodeURIComponent(`Hola ${asesorName}! Tengo dudas sobre mi plan de inversión.`)}`}
            target="_blank" rel="noopener noreferrer"
            style={{
              display: 'inline-flex', alignItems: 'center', gap: 10,
              background: '#fff',
