...
function runSimulation(p: SimulationParams): SimulationResult {
  ...
  const corretajeMonth   = p.guaranteedRentEnabled ? -1 : rentStartMonth;
  ...
  for (let m = 1; m <= totalTableMonths; m++) {
    ...
    const isMonthAfterVacancy = p.guaranteedRentEnabled && isMarketRent && p.vacancyDays > 0
      && m > marketRentStart && (m - marketRentStart) % 12 === 1;
    const corretaje    = (m === corretajeMonth || isMonthAfterVacancy) ? p.monthlyRentCLP * 0.5 : 0;
...
export default function FlujoPage() {
  ...
  const [asesorName, setAsesorName] = useState('Vicente Torres');
  const [asesorPhone, setAsesorPhone] = useState('56994366697');
  ...
  useEffect(() => {
    ...
    try {
      ...
      const urlPhone  = params.get('asesorPhone');
      if (urlAsesor) { setAsesorName(urlAsesor); setAsesorPhone(urlPhone || '56994366697'); setIsClientView(true); }
    } catch {}
  }, []);
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 14, flexWrap: 'wrap', alignItems: 'flex-start', marginTop: 16 }}>
          {isClientView && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, justifyContent: 'center', alignSelf: 'center' }}>
              <a href={`https://wa.me/${asesorPhone}?text=${encodeURIComponent('Hola ' + asesorName + '! Quiero avanzar con la reserva. te paso los numeros de teléfono en breve')}`}
              ...
              <a href={`https://wa.me/${asesorPhone}?text=${encodeURIComponent('Hola ' + asesorName + '! Tengo dudas sobre mi plan de inversión.')}`}
                target="_blank" rel="noopener noreferrer"
